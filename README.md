...
5. Choose one save-time formatting mode:
   ...
   - **t-linter formatter mode** keeps the existing formatter-only workflow:
     ...
     ```
     Set `"t-linter.format.runRuffPipeline": true` if you want this formatter mode to run Ruff fixAll, import organization, Ruff formatting, and then t-linter in one composed formatting transaction.
...
- **Interpolation Type Checking** — optional JSON, YAML, TOML, psycopg SQL interpolation value diagnostics and TDOM component prop interpolation diagnostics through Ty, Pyright, or Pyrefly
