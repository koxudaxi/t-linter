...
```json
{
  "typeChecking": {
    "enabled": true,
    "checker": "pyrefly",
    "command": "/path/to/pyrefly",
    "args": ["lsp"]
  }
}
```

`checker` defaults to `ty`. `command` and `args` are optional. Without them, t-linter uses the selected checker defaults and tries candidates in this order:

1. the selected checker executable in `$VIRTUAL_ENV` or `$CONDA_PREFIX`
2. the selected checker executable in workspace `.venv` or `venv`
3. uv project server: `uv run --project <workspace> --frozen --no-progress <checker-command> <default-args>`
4. PATH fallback: `ty server`, `pyright-langserver --stdio`, or `pyrefly lsp`
